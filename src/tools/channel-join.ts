/**
 * hive_channel_join — Join an existing channel.
 */
...
export function registerChannelJoinTool(pi: ExtensionAPI, state: HiveState): void {
  pi.registerTool({
    name: "hive_channel_join",
    label: "Hive Channel Join",
    description: "Join an existing hive channel.",
    parameters: Type.Object({
      channel: Type.String({ description: "Channel name to join" }),
    }) as any,

    async execute(toolCallId, input, signal, onUpdate, ctx) {
      const params = input as JoinChannelParams;
      ...
      client.send({ type: "channel_join", channel });
      ...
      try {
        ...
        return {
          content: [{ type: "text" as const, text: `Joined #${channel}.` }],
          details: { channel },
        };
      } catch (err: any) {
        return {
    ...
    renderCall(input: any, theme: any) {
      const args = input as JoinChannelParams;
      return new Text(
        theme.fg("toolTitle", theme.bold("hive_channel_join ")) + theme.fg("accent", `#${normalizeChannelName(args.channel || "...")}`),
        0,
        0
      );
    },
